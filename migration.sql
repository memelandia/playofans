...
-- Añadir campos de facturación a models
do $$ begin
...
create index if not exists idx_billing_period_start on billing_records(period_start);
create index if not exists idx_billing_next on models(next_billing_date);
...
-- Trigger updated_at
drop trigger if exists trg_billing_updated on billing_records;
...
drop policy if exists "billing_model_read" on billing_records;
